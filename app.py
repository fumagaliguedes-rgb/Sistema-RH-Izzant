...
def init_db():
    ...
    with con() as db:
        ...
        db.execute('''CREATE TABLE IF NOT EXISTS folha_competencias (
            id INTEGER PRIMARY KEY AUTOINCREMENT, mes INTEGER NOT NULL, ano INTEGER NOT NULL,
            descricao TEXT, status TEXT DEFAULT 'Aberta', criado_em TEXT, fechado_em TEXT,
            UNIQUE(mes, ano))''')
        db.execute('''CREATE TABLE IF NOT EXISTS folha_lancamentos (
            id INTEGER PRIMARY KEY AUTOINCREMENT, competencia_id INTEGER NOT NULL, funcionario_id INTEGER NOT NULL,
            codigo TEXT, descricao TEXT, referencia TEXT, tipo TEXT NOT NULL, valor REAL DEFAULT 0,
            origem TEXT DEFAULT 'Manual', ativo INTEGER DEFAULT 1,
            FOREIGN KEY(competencia_id) REFERENCES folha_competencias(id),
            FOREIGN KEY(funcionario_id) REFERENCES funcionarios(id))''')
        db.execute('''CREATE TABLE IF NOT EXISTS folha_holerites (
            id INTEGER PRIMARY KEY AUTOINCREMENT, competencia_id INTEGER NOT NULL, funcionario_id INTEGER NOT NULL,
            arquivo TEXT, total_proventos REAL DEFAULT 0, total_descontos REAL DEFAULT 0, liquido REAL DEFAULT 0,
            gerado_em TEXT,
            FOREIGN KEY(competencia_id) REFERENCES folha_competencias(id),
            FOREIGN KEY(funcionario_id) REFERENCES funcionarios(id))''')
...
class LoginDialog(tk.Tk):
    def __init__(self, usuario='admin', perfil='Administrador'):
        ...
        tk.Label(card, text='Enterprise v3.8 Folha', bg='white', fg='#9ca3af', font=('Arial', 9)).pack(pady=(0,14))
...
class App(tk.Tk):
    def __init__(self, usuario='admin', perfil='Administrador'):
        ...
        self.usuario = usuario
        self.perfil = perfil
        self.title(APP_NAME + ' - Enterprise v3.8 Folha')
    ...
    def build_ui(self):
        ...
        tk.Label(logo_box, text='Enterprise v3.4', bg='#111827', fg='#9ca3af',
        ...
        self.tab_pdf=ttk.Frame(self.nb); self.tab_folha=ttk.Frame(self.nb); self.tab_backup=ttk.Frame(self.nb); self.tab_rel=ttk.Frame(self.nb)
        ...
        for frame, titulo in [
            ...
            (self.tab_folha,'Folha de Pagamento'), (self.tab_backup,'Backup'), (self.tab_rel,'Relatórios'), (self.tab_logs,'Auditoria'),
        ...
        self.grupos_menu = {
            ...
            'Gestão de Pessoas': [
                ...
                ('Folha de Pagamento', 'Competências, lançamentos e holerites', self.tab_folha, '💰'),
        ...
        self.build_inicio(); self.build_grupo_placeholder(); self.build_empresa(); self.build_func(); self.build_setores(); self.build_jornadas(); self.build_escalas(); self.build_feriados(); self.build_ocorrencias(); self.build_ferias(); self.build_banco_horas(); self.build_documentos(); self.build_epis(); self.build_exames(); self.build_agenda(); self.build_central_pdfs(); self.build_assistente(); self.build_pdf(); self.build_folha_pagamento(); self.build_backup(); self.build_relatorios(); self.build_logs(); self.build_usuarios(); self.build_atualizador(); self.build_dev()
    ...
    def _gerar_pdf_ferias_resumo(self, tipo, dados, destino_pdf):
        ...
        abono_valor = moeda(val('VALOR_ABONO'))
        total_bruto = moeda(val('TOTAL_BRUTO'))
        inss = moeda(val('INSS_ESTIMADO'))
        liquido = moeda(val('LIQUIDO_ESTIMADO'))
        if total_bruto == '0,00':
            total_bruto = moeda_br(valor_moeda_para_float(valor_ferias) + valor_moeda_para_float(um_terco)).replace('R$','').strip()
        if liquido == '0,00':
            liquido = total_bruto
        ...
        c.saveState()
        try:
            c.setFillAlpha(0.06); c.setStrokeAlpha(0.06)
        except Exception:
            pass
        try:
            if os.path.exists(LOGO_APP):
        ...
        # Demonstrativo com cabeçalho e linhas fixas
        demo_top = base_y - 4
        ...
        for i, h in enumerate(headers): txt((cols[i]+cols[i+1])/2, table_top-11, h, 7.8, True, 'center')
        lancs = [
        ...
        if str(dias_abono).strip() not in ('', '0', '0,00'):
        ...
        for cod, desc, ref, prov, descv in lancs[:7]:
            yrow -= row_h
            rect(L, yrow, BW, row_h)
            for x in cols[1:-1]: line(x, yrow, x, yrow+row_h)
            txt(L+6, yrow+5, cod, 7.7)
            txt(L+47, yrow+5, desc[:45], 7.7)
        ...
        y -= 55
        rect(L+10, y, 92, 23); rect(L+102, y, BW-112, 23)
        txt(L+56, y+13, 'Valor por', 7, False, 'center'); txt(L+56, y+5, 'extenso', 7, False, 'center')
    ...
    def build_folha_pagamento(self):
        ...
        self.folha_nb.add(self.folha_tab_lanc, text='➕ Lançamentos da Folha')
    ...
    def _build_folha_lancamentos_tab(self):
        ...
        self.folha_evento_combo.grid(row=0, column=1, columnspan=4, sticky='ew', padx=6, pady=5)
        ttk.Label(form, text='Quantidade/Ref.').grid(row=0, column=5, sticky='w', padx=6, pady=5)
        ...
        ttk.Label(form, text='Forma').grid(row=1, column=0, sticky='w', padx=6, pady=5)
        self.folha_lanc_forma = tk.StringVar(value='Valor R$')
        ttk.Combobox(form, textvariable=self.folha_lanc_forma, values=['Valor R$','Percentual do salário','Percentual dos proventos'], state='readonly', width=20).grid(row=1, column=1, columnspan=2, sticky='ew', padx=6, pady=5)
        ttk.Label(form, text='Valor ou %').grid(row=1, column=3, sticky='w', padx=6, pady=5)
        ...
        ttk.Label(form, text='Ex.: 212,00 ou 10 para 10%').grid(row=1, column=5, columnspan=2, sticky='w', padx=6, pady=5)
        ...
        ttk.Combobox(form, textvariable=self.folha_aplicar_tipo, values=['Funcionário','Setor','Função','Todos'], state='readonly', width=14).grid(row=2, column=1, sticky='ew', padx=6, pady=5)
        ttk.Label(form, text='Alvo').grid(row=2, column=2, sticky='w', padx=6, pady=5)
    ...
    def _folha_get_competencia_id(self):
        ...
        with con() as db:
            row=db.execute('SELECT id FROM folha_competencias WHERE mes=? AND ano=?',(mes,ano)).fetchone()
            if row: return row[0]
            cur=db.execute('INSERT INTO folha_competencias(mes,ano,descricao,status,criado_em) VALUES(?,?,?,?,?)',
    ...
    def _folha_calcular_valor_evento(self, evento, funcionario):
        ...
        ref_txt=self.folha_lanc_ref.get().strip() or (ref_padrao or '')
        forma = self.folha_lanc_forma.get() if hasattr(self, 'folha_lanc_forma') else 'Valor R$'
        valor_digitado=parse_moeda_br(self.folha_lanc_valor.get())
        salario=float(funcionario.get('salario') or 0)
        try:
            qtd=float(str(ref_txt).replace(',','.').replace('%',''))
        except Exception:
            qtd=0.0

        # Lançamento em percentual: o campo "Valor ou %" representa a porcentagem.
        # O valor calculado é gravado no lançamento para manter o holerite simples e auditável.
        ...
        if forma == 'Percentual dos proventos' and valor_digitado > 0:
            ref_txt = f'{valor_digitado:.4f}%'.replace('.', ',')
            # Nesta etapa, usa salário base como referência inicial. Após o cálculo da folha,
            # o valor fica fixado no lançamento para evitar variação inesperada no holerite.
            return ref_txt, round(salario * valor_digitado / 100.0, 2)
    ...
    def aplicar_lancamento_folha(self):
        ...
        with con() as db:
            for f in funcs:
                ...
                if valor <= 0: continue
                origem_lanc = 'Manual'
                if hasattr(self, 'folha_lanc_forma') and self.folha_lanc_forma.get() != 'Valor R$':
                    origem_lanc = 'Manual - Percentual'
                db.execute('''INSERT INTO folha_lancamentos(competencia_id,funcionario_id,codigo,descricao,referencia,tipo,valor,origem,ativo)
                              VALUES(?,?,?,?,?,?,?,?,1)''', (comp_id,f['id'],cod,desc,ref,tipo,valor,origem_lanc))
                qtd += 1
    ...
    def carregar_folha_lancamentos(self):
        ...
        with con() as db:
            rows=db.execute('''SELECT l.id,f.nome,l.codigo,l.descricao,l.referencia,l.tipo,l.valor,l.origem
                               ...
                               WHERE l.competencia_id=? AND l.ativo=1 AND COALESCE(l.origem,'') LIKE 'Manual%'
    ...
    def _gerar_holerite_pdf(self, f, destino):
        ...
        c = canvas.Canvas(destino, pagesize=A4); W, H = A4; L, R = 18, W - 18; block_w = R - L; block_h = 305; y1 = H - 20 - block_h; y2 = 78
        ...
        def draw_via(y0):
            ...
            row_y=table_top-header_h-11; max_rows=10
            ...
            txt(x0+392,totals_y-18,'SALÁRIO LÍQUIDO',8.4,True); txt(R-8,totals_y-18,f'R$ {m(liquido)}',8.8,True,'right')
            base_y=y0+52
